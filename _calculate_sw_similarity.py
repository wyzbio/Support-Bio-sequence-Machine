...
def smithwaterman_train_similarity(sequence_pair):
...
def smithwaterman_test_similarity(query, target_list):
    aligner = PairwiseAligner()
    aligner.mode = 'local'
    # aligner.match_score = 1
    # aligner.mismatch_score = -1
    ...
    for target in target_list:
        ...
        similarity = alignments.score
        similarities.append(similarity)
    return similarities

# @timer
def compute_smithwaterman_train_kernel(sequences, process_num):
    ...
    with Pool(processes=process_num) as pool:
        scores = pool.map(smithwaterman_train_similarity, sequence_pairs)
...
# @timer
def compute_smithwaterman_test_kernel(queries, targets, process_num):
    similarity_matrix = [[None]*len(targets) for _ in range(len(queries))]
    with Pool(processes=process_num) as pool:
        results = []
        for i, query in enumerate(queries):
            result = pool.apply_async(smithwaterman_test_similarity, args=(query, targets))
